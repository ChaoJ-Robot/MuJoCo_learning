...
import numpy as np
# from gen3_robot.resource.models.kinova_gen3.base_controller import BaseController
...
# 添加路径以导入base_controller - 修复ROS2环境中的路径问题
current_dir = os.path.dirname(os.path.abspath(__file__))
# 在ROS2包环境中，需要向上三级目录才能找到resource
package_root = os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))
resource_path = os.path.join(package_root, 'resource')
sys.path.insert(0, resource_path)

try:
    from models.kinova_gen3.base_controller import BaseController
except ImportError:
    # 备用导入方式
...
if __name__ == "__main__":
    ...
    xml_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
                           'resource', 'models', 'kinova_gen3', 'gen3.xml')
    robot = Gen3Robot(xml_path)
    robot.set_up()
